...
function invalidateBudgetActualComparison() { rrllBudgetActualComparisonData = null; document.getElementById("budgetActualComparisonResult")?.classList.add("budget-form-hidden"); }
function renderBudgetActualComparisonSelectors() {
  const scenario = document.getElementById("budgetActualComparisonScenario"); const year = document.getElementById("budgetActualComparisonYear"); const cutoff = document.getElementById("budgetActualComparisonCutoff"); const listYear = document.getElementById("budgetActualListYear");
  if (!scenario || !year || !cutoff || !listYear) return;
  const selected = scenario.value; scenario.innerHTML = rrllBudgetScenarios.map(item => `<option value="${rrllBudgetEscape(item.id)}">${rrllBudgetEscape(item.name)}</option>`).join("");
  if (rrllBudgetScenarios.some(item => item.id === selected)) scenario.value = selected;

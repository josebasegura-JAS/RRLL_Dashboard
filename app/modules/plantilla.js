...
(function () {
  ...
  function renderPlantillaPagination(totalRows) {
    ...
    pagination.innerHTML = `
      <button type="button" class="rrll-pro-tool-button" onclick="plantillaPreviousPage()" ${plantillaCurrentPage <= 1 ? 'disabled' : ''}>Anterior</button>
      <span>Página ${plantillaCurrentPage} de ${totalPages} · ${totalRows} registros</span>
      <button type="button" class="rrll-pro-tool-button" onclick="plantillaNextPage()" ${plantillaCurrentPage >= totalPages ? 'disabled' : ''}>Siguiente</button>
    `;
  }

const BudgetDomain = (() => {
  ...
  function calculateBudgetTicketGroupMonth(group = {}, scenario = {}, month, context = {}) {
    ...
    const ticketDays = typeof context.countTicketDaysForCalendar === "function"
      ? normalizeBudgetNumber(context.countTicketDaysForCalendar(calendar, Number(getValue(scenario, "año", "year")), normalizedMonth))
      : 0;
  ...
  function calculateBudgetTicketGroupYear(group = {}, scenario = {}, context = {}) {
    const byMonth = MONTHS.map(month => ({ month, totalTicket: calculateBudgetTicketGroupMonth(group, scenario, month, context) }));
    return { totalTicket: byMonth.reduce((total, item) => total + item.totalTicket, 0), byMonth };
  }

  function calculateBudgetTicketScenarioMonth(groups = [], scenario = {}, month, context = {}) {
    const byGroup = (Array.isArray(groups) ? groups : []).map(group => ({ group, totalTicket: calculateBudgetTicketGroupMonth(group, scenario, month, context) }));
    return { totalTicket: byGroup.reduce((total, item) => total + item.totalTicket, 0), byGroup };
  }

  function calculateBudgetTicketScenarioYear(groups = [], scenario = {}, context = {}) {
    const byMonth = MONTHS.map(month => ({ month, ...calculateBudgetTicketScenarioMonth(groups, scenario, month, context) }));
    const byGroup = (Array.isArray(groups) ? groups : []).map(group => ({ group, ...calculateBudgetTicketGroupYear(group, scenario, context) }));
    return { totalTicket: byMonth.reduce((total, item) => total + item.totalTicket, 0), byMonth, byGroup };
  }

  function calculateBudgetScenarioMonth({ manualItems = [], ticketGroups = [], scenario = {}, month, context = {} } = {}) {
    ...
    const ticket = calculateBudgetTicketScenarioMonth(ticketGroups, scenario, month, context);
  ...
  function calculateBudgetScenarioYear({ manualItems = [], ticketGroups = [], scenario = {}, context = {} } = {}) {
    const byMonth = MONTHS.map(month => calculateBudgetScenarioMonth({ manualItems, ticketGroups, scenario, month, context }));
    const byManualItem = (Array.isArray(manualItems) ? manualItems : []).map(item => ({ item, totalManual: calculateBudgetManualItemYear(item) }));
    const ticket = calculateBudgetTicketScenarioYear(ticketGroups, scenario, context);
    const totalManual = byManualItem.reduce((total, item) => total + item.totalManual, 0);
    return { totalManual, totalTicket: ticket.totalTicket, totalScenario: totalManual + ticket.totalTicket, byMonth, byManualItem, byGroup: ticket.byGroup };
  }

  return Object.freeze({
    normalizeBudgetNumber, normalizeBudgetRate,
    ...
    calculateBudgetScenarioMonth, calculateBudgetScenarioYear
  });
})();

if (typeof window !== "undefined") window.BudgetDomain = BudgetDomain;

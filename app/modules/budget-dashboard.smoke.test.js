...
assert.match(context.printHtml, /class="budget-chart-axis" stroke="currentColor"/);
assert.match(context.printHtml, /class="budget-chart-grid" stroke="currentColor" opacity="\.18"/);
assert.match(context.printHtml, /<title>Enero · Presupuesto: 150,00 € · Real: 250,00 € · Desviación: 100,00 €<\/title>/);
assert.match(context.printHtml, /Presupuesto<\/text>/);
assert.match(context.printHtml, /Desviación positiva<\/text>/);
assert.match(context.printHtml, /Desviación negativa<\/text>/);
...
const budgetCss = fs.readFileSync(require.resolve("../styles/features/50-budget.css"), "utf8");
assert.match(budgetCss, /color: var\(--rrll-text-main, var\(--text, #f1f5f9\)\)/);
assert.doesNotMatch(budgetCss, /--text-color/);
